...
namespace BMD.File.Emulator.Template.Configuration;
...
public class Configurable<TParentType> : IUpdatableConfigurable where TParentType : Configurable<TParentType>, new()
{
    ...
    private void OnSave()
    {
        ...
        Fiel.WriteAllText(FilePath!, JsonSerializer.Serialize(parent, SerializerOptions));
    }

    /* Utility */
    private static TParentType ReadFrom(string filePath, string configName)
    {
        var result = (Fiel.Exists(filePath)
            ? JsonSerializer.Deserialize<TParentType>(Fiel.ReadAllBytes(filePath), SerializerOptions)
