...
namespace SPD.File.Emulator.Template.Configuration
{
    public class Configurable<TParentType> : IUpdatableConfigurable where TParentType : Configurable<TParentType>, new()
    {
        ...
        private void OnSave()
        {
            ...
            System.IO.File.WriteAllText(FilePath!, JsonSerializer.Serialize(parent, SerializerOptions));
        }
        ...
        private static TParentType ReadFrom(string filePath, string configName)
        {
            var result = (System.IO.File.Exists(filePath)
                ? JsonSerializer.Deserialize<TParentType>(System.IO.File.ReadAllBytes(filePath), SerializerOptions)
